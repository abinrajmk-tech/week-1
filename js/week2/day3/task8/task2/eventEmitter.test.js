...
test("Event Emitter: off function ", () => {
    const mockCallback = jest.fn();
    users.on("userAdded", mockCallback);
    users.emit("userAdded", 1);
    expect(mockCallback).toHaveBeenCalledWith(1);
    users.off("userAdded", mockCallback);
    expect(users.eventMap.get("userAdded")).not.toEqual([mockCallback]);
});
test("Event Emitter: once function", () => {
    const mockCallback = jest.fn();
    const userRemoved = () => "user Removed";
    users.once("userRemoved", userRemoved);
    users.on("*", mockCallback);
    users.emit("userRemoved", 1);
    expect(users.eventMap.get("userRemoved")).not.toEqual([
        userRemoved,
        mockCallback,
    ]);
});
